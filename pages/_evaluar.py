...
# --- VALIDACIÓN DEL ENLACE (COMO TU if rol == "bodega") ---
params = st.query_params
cedula_empleado = params.get("cedula", [None])[0]
token = params.get("token", [None])[0]

if not cedula_empleado or not token:
    st.error("❌ Enlace inválido o incompleto.")
    st.stop()
