    ...
    if empleados_disponibles:
        ...
        with col_card_der:
            ...
            with st.form("form_edicion"):
                ...
                updates["NOMBRE COMPLETO"] = c_a.text_input("Nombre Completo", value=datos.get("NOMBRE COMPLETO", ""), key="nombre_completo")
                cedula_disabled = c_b.text_input("Cédula (ID Único)", value=datos.get("CEDULA", ""), disabled=True, key="cedula")
                updates["CARGO"] = c_a.text_input("Cargo", value=datos.get("CARGO", ""), key="cargo")
                updates["DEPARTAMENTO"] = c_b.text_input("Departamento", value=datos.get("DEPARTAMENTO", ""), key="departamento")
                updates["JEFE_DIRECTO"] = c_a.text_input("Jefe Directo", value=datos.get("JEFE_DIRECTO", ""), key="jefe_directo")
                updates["SEDE"] = c_b.text_input("Sede", value=datos.get("SEDE", ""), key="sede")
                updates["CORREO"] = c_a.text_input("Correo", value=datos.get("CORREO", ""), key="correo")
                updates["CELULAR"] = c_b.text_input("Celular", value=str(datos.get("CELULAR", "")), key="celular")
                updates["DIRECCIÓN DE RESIDENCIA"] = c_a.text_input("Dirección de Residencia", value=datos.get("DIRECCIÓN DE RESIDENCIA", ""), key="direccion_residencia_a")
                updates["BANCO"] = c_b.text_input("Banco", value=datos.get("BANCO", ""), key="banco")
                # CAMPOS NUEVOS
                updates["FECHA_INGRESO"] = c_a.text_input("Fecha de Ingreso", value=datos.get("FECHA_INGRESO", ""), key="fecha_ingreso")
                updates["FECHA_NACIMIENTO"] = c_b.text_input("Fecha de Nacimiento", value=datos.get("FECHA_NACIMIENTO", ""), key="fecha_nacimiento")
                updates["ESTADO_CIVIL"] = c_a.text_input("Estado Civil", value=datos.get("ESTADO_CIVIL", ""), key="estado_civil")
                updates["HIJOS"] = c_b.text_input("Hijos", value=datos.get("HIJOS", ""), key="hijos")
                updates["DIRECCION"] = c_a.text_input("Dirección", value=datos.get("DIRECCION", ""), key="direccion")
                updates["ESTADO"] = c_b.selectbox("Estado", ["Activo", "Inactivo"], index=0 if datos.get("ESTADO", "Activo") == "Activo" else 1, key="estado")
                updates["SALARIO APORTES"] = c_a.text_input("Salario", value=str(datos.get("SALARIO APORTES", "")), key="salario_aportes")
                updates["ESTADO_CONTRATO"] = c_b.text_input("Estado Contrato", value=datos.get("ESTADO_CONTRATO", ""), key="estado_contrato")
